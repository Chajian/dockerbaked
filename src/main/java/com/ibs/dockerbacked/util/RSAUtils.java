...
//    }
    ...
    public static KeyStore createKeys() throws NoSuchAlgorithmException {
        ...
        if(keyStore==null) {
            ...
            keyStore = new KeyStore(publicKey, privateKey);
        }
        return keyStore;
    }
    ...
    private static byte[] encryptByPublicKey(byte[] data, byte[] key) throws NoSuchAlgorithmException,
            InvalidKeySpecException, NoSuchPaddingException, BadPaddingException, InvalidKeyException, IllegalBlockSizeException {
        //实例化密钥工厂
        KeyFactory keyFactory = KeyFactory.getInstance(RSA_ALGORITHM);
